...
const Farming = () => {
  ...
  for (let x=0;x<producer.length;x++) {
    farmingTypes = [...farmingTypes, ...(producer[x].crops?.split(',')).map(item => item.trim())]
  }
            ...
            datasets: [
              {
                ...
                data: noDuplicatesFarmingTypes.map(item => getOccurrence(farmingTypes.map(farming => farming), item)),
                backgroundColor: noDuplicatesFarmingTypes.map(()  => generateRandomColor()),
                borderColor: "black",
                borderWidth: 1,
              },
            ],
